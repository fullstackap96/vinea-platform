...
import { googleCalendarUpdateRouteTestInternals } from '@/app/api/google/calendar-event/update/route'
...
const routePath = join(process.cwd(), 'app', 'api', 'google', 'calendar-event', 'update', 'route.ts')
...
describe('Google Calendar update route parish context', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('uses resolved primary parish context when no active parish cookie exists', async () => {
    ...
    const result =
      await googleCalendarUpdateRouteTestInternals.resolveGoogleCalendarUpdateParishContext(
        supabase as never,
        null
      )
  ...
  it('checks request parish ownership through the parishioner row', () => {
    expect(
      googleCalendarUpdateRouteTestInternals.parishionerMatchesParish(
        { parish_id: 'parish-2' },
        'parish-2'
      )
    ).toBe(true)
  ...
  it('allows only blank or selected-parish calendar ids on linked requests', () => {
    expect(
      googleCalendarUpdateRouteTestInternals.requestCalendarMatchesSelectedIntegration(
  ...
  it('keeps Google patching behind staff auth, active parish scope, request scope, and selected integration', () => {
    ...
    expectBefore(source, 'const parishContext = await resolveGoogleCalendarUpdateParishContext', "const { data: reqRow")
  ...
  it('logs unexpected update failures through the shared safe logger', () => {
    ...
    expect(source).toContain('function logGoogleCalendarUpdateEventError')
    expect(source).toContain("logServerError('[google-calendar-update-event] unexpected failure', error")
    expect(source).toContain("route: '/api/google/calendar-event/update'")
    expect(source).toContain('hasParishId: Boolean(parishId)')
    expect(source).not.toContain('console.error')
    expect(source).not.toContain('GOOGLE CALENDAR UPDATE EVENT ERROR (technical):')
