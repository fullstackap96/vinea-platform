...
import {
  PARISH_INTAKE_QUEUE_INTENTION_SELECT,
  loadParishIntakeQueue,
} from './loadParishIntakeQueue'
...
function createParishIntakeQueueSupabaseMock() {
...
describe('loadParishIntakeQueue active parish context', () => {
  it('passes the validated active parish id to dashboard request loading and mass intentions filtering', async () => {
    const { supabase, calls } = createParishIntakeQueueSupabaseMock()
      ...
      expect.arrayContaining([
        {
          ...
          method: 'select',
          args: [PARISH_INTAKE_QUEUE_INTENTION_SELECT],
        },
        { table: 'mass_intentions', method: 'eq', args: ['is_fulfilled', false] },
        { table: 'mass_intentions', method: 'eq', args: ['parish_id', 'parish-2'] },
      ])
    )
    expect(PARISH_INTAKE_QUEUE_INTENTION_SELECT).not.toContain('notes')
    expect(PARISH_INTAKE_QUEUE_INTENTION_SELECT).not.toContain('intention_text')
    expect(PARISH_INTAKE_QUEUE_INTENTION_SELECT).not.toContain('requested_date')
    expect(PARISH_INTAKE_QUEUE_INTENTION_SELECT).not.toContain('*')
  })
  ...
  it('returns a context error before loading dashboard requests when active parish context fails', async () => {
    const { supabase } = createParishIntakeQueueSupabaseMock()
