...
const pagePath = join(process.cwd(), 'app', 'dashboard', 'households', 'duplicates', 'page.tsx')
const clientPath = join(
  ...
  'households',
  'duplicates',
  'HouseholdDuplicatesPageClient.tsx'
)
const routePath = join(process.cwd(), 'app', 'api', 'households', 'duplicates', 'route.ts')
const evidencePath = join(
  process.cwd(),
  'docs',
  'HOUSEHOLD_DUPLICATES_SELECTED_PARISH_SCOPE_UX_20260630.md'
)

describe('household duplicates selected parish scope UX', () => {
  it('passes the validated active parish name from the server page into the duplicate review client', () => {
    ...
    expect(page).toContain(
      '<HouseholdDuplicatesPageClient activeParishName={activeParishName} />'
    )
  })
  ...
  it('shows a visible staff-facing selected parish label before duplicate merge actions', () => {
    ...
    expect(client).toContain('Household duplicate review is scoped to')
    expect(client).toContain('{activeParishName}')
    expect(client.indexOf('Household duplicate review is scoped to')).toBeLessThan(
  ...
  it('documents the non-production-safe scope without claiming browser QA or runtime policy changes', () => {
    ...
    for (const expected of [
      ...
      'duplicate households were not merged',
