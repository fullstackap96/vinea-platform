...
const confirmation = process.env.VINEA_MEMBERSHIP_RLS_DISPOSABLE_CONFIRM
const reusableDisposableProjectConfirmation = process.env.VINEA_ALLOW_REUSED_DISPOSABLE_PROJECT

const confirmationValue = 'MEMBERSHIP_AWARE_RLS_DISPOSABLE_VALIDATION'
...
if (confirmation !== confirmationValue) {
  throw new Error(
    `VINEA_MEMBERSHIP_RLS_DISPOSABLE_CONFIRM=${confirmationValue} is required.`
  )
}
...
const forwardPath = join(root, 'docs', 'sql', 'membership_aware_operational_rls_migration_candidate.sql')
