  ...
  try {
    ...
    const body =
      parsedBody.ok && parsedBody.value && typeof parsedBody.value === 'object'
        ...
        : null
    const confirmedServiceAt = optionalDateTime(body?.confirmedServiceAt)

    if (confirmedServiceAt === undefined) {
      return NextResponse.json(
        { ok: false, error: 'Invalid confirmed funeral service update.' },
        { status: 400 }
      )
    }
    const auditOperation = confirmedServiceAt ? 'set' : 'cleared'
    ...
    const { data: updatedDetail, error } = await admin
      .from('funeral_request_details')
      .update({ confirmed_service_at: confirmedServiceAt })
      .eq('request_id', access.requestId)
      .select('request_id')
      .maybeSingle()
    ...
    await writeAuditEvent({
      ...
      metadata: {
        ...
        scheduleKind: 'confirmed_funeral_service',
    ...
    logServerError('[request-confirmed-funeral-service] update failed', error, {
      route: '/api/requests/[id]/confirmed-funeral-service',
      hasRequestId: Boolean(requestId),
      activeParishCookiePresent: Boolean(activeParishId),
    })
    return NextResponse.json(
      { ok: false, error: 'Could not update confirmed funeral service.' },
      { status: 500 }
    )
  }
}
