...
export async function PATCH(request: NextRequest, context: RouteParams) {
  ...
  try {
    ...
    if (!parsedBody.ok && parsedBody.reason === 'too_large') {
      return NextResponse.json({ ok: false, error: 'AI summary is too large.' }, { status: 413 })
    }
    const body =
    ...
    if (typeof aiSummary !== 'string') {
      return NextResponse.json({ ok: false, error: 'Invalid AI summary update.' }, { status: 400 })
    }
    ...
    await writeAuditEvent({
      ...
      action: 'request.ai_summary.updated',
      targetType: 'request',
      targetId: access.requestId,
      metadata: {
        source: 'staff_request_detail',
        outputLength: aiSummary.length,
    ...
    logServerError('[request-ai-summary] update failed', error, {
      route: '/api/requests/[id]/ai-summary',
      hasRequestId: Boolean(requestId),
      activeParishCookiePresent: Boolean(activeParishId),
    })
    return NextResponse.json(
      { ok: false, error: 'Could not update AI summary.' },
      { status: 500 }
    )
  }
}
