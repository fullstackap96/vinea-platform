...
Latest release-candidate rollout evidence and repair: exact commit `9899f8e7fc6fed898e13a73f0beb522dcacd848b` is `READY` in Vercel Preview, exact-head CI passed, and authenticated read-only smoke passed staff sign-in, active-parish switching, Onboarding, Imports, People duplicate review, and Communications Center. Its `/api/health` false-negative came from checking only `NEXT_PUBLIC_APP_URL` while Vinea's established origin policy already trusts exact Vercel system deployment hostnames. Immutable follow-up commit `19b601df046de06a6a52f214e24adcdeba05b577` aligns health with that policy and preserves fail-closed exact-HTTPS parsing. The complete 15-check local gate passed with `832` files / `3,546` tests and a credential-free `56`-page build. Next priority is exact-head CI, a fresh Preview deployment, and a green `checks.schema` rerun. Merge, production deployment, production RLS, monitoring, exports, public-intake routing, customer-facing AI, and public trust claims remain separately gated.
