...
Latest safe selected-parish reliability slice (2026-07-20): Daily Work Hub workflow SLA settings can no longer hang indefinitely, retain prior-parish rules during a switch, or let a late prior-parish response overwrite the selected parish. Commit `0392f5110a3cdc4ff0040ebcbcd50db9751eaa52` adds a 15-second read deadline, default reset, latest-load ownership, response-parish validation, and cleanup invalidation without changing the authorized settings API or adding writes. Focused coverage passed `10` files / `54` tests; the complete `15 / 15` contract passed with zero secret findings, zero vulnerabilities, both TypeScript scopes, lint, `870` files / `3,727` tests, and the credential-free `56`-page build. Aggregate `B06BC97423E5C68DC1E0949CF6750906EEBD27CE4274BFE99706180084A68886` binds `1,519` tracked release-source files. Engineering remains estimated at `99.8%`; rollout readiness remains `87.5%`. Live two-parish browser evidence and exact-head remote publication/CI/Preview remain separate. No production-sensitive action occurred. See `docs/DAILY_WORK_HUB_WORKFLOW_SETTINGS_SCOPE_BOUNDARY_20260720.md`.

Latest safe release-integrity slice (2026-07-20): exact-head CI run `29791367802` found one clean-checkout source-manifest evidence mismatch after `3,722` tests passed. The assertion had used mutable working-tree counts and was accidentally satisfied locally by four unrelated untracked scripts. Commit `9e15d6b2ba1788c39fc7bf8cf8bd82f0265cfccd` now validates the immutable tracked-head manifest instead; aggregate `1AA7EC5167D88A91314CA3D2CF029C2CAB9111BD241FB599587A8F4B7AC7CEC7` binds `1,518` files. Focused coverage passed `1` file / `6` tests, and the complete `15 / 15` contract passed with zero secret findings, zero vulnerabilities, both TypeScript scopes, lint, `869` files / `3,723` tests, and the credential-free `56`-page build. Separate exact-head publication, clean-checkout CI, and non-production Preview verification remain pending. No runtime, merge, deployment, production, migration, RLS, or sensitive-feature action occurred.

Latest safe reliability slice (2026-07-20): Request Documents upload, review, and family-link creation now freeze every document write after timeout, transport uncertainty, malformed success, or failed authoritative list reload. Confirmed rejection remains retryable, confirmed upload/review success follows checked reload, a created link survives clipboard failure, and no mutation replays automatically. Commit `8544263fd91691e6234d4111e71a703180c7fbdb`, tracked aggregate `F7C1111468121579492A38F46D317997755AF0F13B06802226F2303F0C934007`, focused `7` files / `34` tests, and the complete `15 / 15` release contract (`869` files / `3,723` tests plus credential-free `56`-page build) bind the result. Engineering remains estimated at `99.8%`; rollout readiness remains `87.5%` because exact-head CI/Preview and human production gates remain external. Existing storage, token, and authorization semantics are unchanged, and no production-sensitive action occurred. See `docs/REQUEST_DOCUMENT_CONFIRMATION_RECOVERY_BOUNDARY_20260720.md`.

Latest safe reliability slice (2026-07-20): the staff-reviewed Baptism certificate action now has a 60-second browser confirmation boundary and synchronous single flight. Explicit server rejection remains retryable; any uncertain delivery, malformed PDF acknowledgement, or empty PDF requires refresh and certificate-activity review before another attempt, and no generation request replays automatically. Commit `b85ea09123fe4abc9d6cf45b999d455f38352bcb`, tracked aggregate `BCE10A325BC6A9E48BBF8B324DFA691A5EA9C1F122189B2B630C48F6ADDDF60D`, focused `6` files / `22` tests, and the complete `15 / 15` release contract (`869` files / `3,721` tests plus credential-free `56`-page build) bind the result. Engineering remains estimated at `99.8%`; rollout readiness remains `87.5%` because exact-head CI/Preview and human production gates remain external. Existing certificate scope and server authorization are unchanged, and no certificate or production action occurred. See `docs/RECORD_CERTIFICATE_CLIENT_CONFIRMATION_DEADLINE_BOUNDARY_20260720.md`.

Latest safe reliability slice (2026-07-20): Onboarding completion now confirms the selected parish, applies a 60-second browser deadline, and requires an authoritative same-parish reload with a persisted completion timestamp before staff see success. Ambiguous outcomes freeze retry until refresh/review, and no write replays automatically. Focused coverage passed `10` files / `38` tests; the complete `15`-check release contract passed with zero secret findings, zero vulnerabilities, both TypeScript scopes, lint, `868` test files / `3,717` tests, and the credential-free `56`-page build. Engineering remains estimated at `99.8%`; rollout readiness remains `87.5%` because exact-head CI/Preview and human production gates remain external. See `docs/ONBOARDING_COMPLETION_CONFIRMATION_DEADLINE_BOUNDARY_20260720.md`.

Latest safe Daily Brief reliability slice (2026-07-20): the staff-initiated selected-parish send now has a 30-second browser confirmation boundary and reports success only after `ok: true` plus a non-empty provider message id. Timeout or acknowledgement uncertainty gives inbox-first guidance, a selected-parish switch suppresses stale settlement, and a deliberate same-parish retry reuses the existing opaque attempt without automatic replay. Commit `fe0c75fbe6cdb674153282babf74f6e7573407a3`, tracked aggregate `0AC717E566E754D32D5DC0C8779DA67661A00B1203179FEBCF390849EF396B72`, focused `9` files / `41` tests, and the complete `15 / 15` release contract (`866` files / `3,710` tests plus credential-free `56`-page build) bind the result. Engineering remains estimated at `99.8%`; rollout readiness remains `87.5%` because live non-production uncertainty/retry evidence and production-sensitive human gates remain separate. Existing provider and authorization behavior is unchanged, and no email or production action occurred.

Latest safe Public Intake Routing Settings reliability slice (2026-07-20): metadata, domain, DNS verification, and one-time token mutations now use a 60-second confirmation boundary and require a valid acknowledgement plus authoritative selected-parish reload before reporting success. Explicit server rejections remain retryable; ambiguous transport, malformed success, or failed reload freezes the complete management surface until refresh/review; stale parish results and prior-parish one-time tokens are discarded; and no mutation replays automatically. Commit `f0b7d6201657785cfe731527e09dd6a7ce698d79`, tracked aggregate `9610711F2CA49207F321D827B95719F6E859C7A318289DA9B1FE9430600DA92D`, focused `15` files / `74` tests, and the complete `15 / 15` release contract (`864` files / `3,704` tests plus credential-free `56`-page build) bind the result. Engineering remains estimated at `99.8%`; rollout readiness remains `87.5%` because live non-production confirmation evidence and production-sensitive human gates remain separate. Runtime public routing remains unchanged and no production or routing mutation occurred.
