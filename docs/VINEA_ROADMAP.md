...
Latest safe membership-primary request access hardening: when staff have no active-parish cookie, request detail and document access now use their authenticated primary parish membership before considering the isolated legacy global-primary compatibility path. Membership-resolution errors fail closed, and existing request ownership checks remain authoritative. Focused coverage passed 5 files / 25 tests; the complete 15-check local release contract passed with zero secret findings across 2,842 text files, zero vulnerabilities, all evidence gates, both TypeScript scopes, lint, 836 test files / 3,566 tests, and the credential-free Next.js 16 56-page build. No migration, operational RLS, production, provider, storage, or data action occurred. Evidence: `docs/REQUEST_ACCESS_MEMBERSHIP_PRIMARY_FALLBACK_HARDENING_20260720.md`.
