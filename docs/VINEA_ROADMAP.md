...
Latest controlled-rollout intake milestone: the remaining human release inputs now have one repository-owned worksheet and mechanical gate. `docs/CONTROLLED_PRODUCTION_ROLLOUT_HUMAN_INTAKE_20260714.md` captures label-only owners, channels, read-only production-safe fixtures, an exact low-traffic window, rollback observation, and scope confirmations; `scripts/check-controlled-production-rollout-intake.mjs` binds the merged SHA plus candidate/rollback deployments and rejects credential-like content without printing entered values. The current decision is intentionally `NO_GO_MISSING_HUMAN_INPUT`. Next priority is human completion of `26` fields and `7` confirmations followed by a separate exact approval. This work does not authorize promotion, production smoke, migrations, flags, or any separately locked production capability.
