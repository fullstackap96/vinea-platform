...
Latest verified release commit boundary: `npm run check:release-commit-scope` is read-only and proposes only source/configuration plus Markdown/SQL/sanitized JSON evidence. Sales, CSV, generated output, binaries, temporary artifacts, environment secrets, and local logs are excluded. Staging and commit remain owner-gated.

Latest verified release identity boundary: `npm run check:release-source-manifest` computes a deterministic SHA-256 aggregate over tracked and non-ignored untracked release source without printing paths, contents, environment files, or secrets. It binds evidence to the current dirty-worktree source surface but does not replace the required clean immutable Git commit and final rerun.

Latest verified local release boundary: the complete 12-command release runner passes only from a process-clean environment and correctly refuses QA/prototype runtime residue. The current refresh passes all evidence gates, TypeScript, lint, 806 test files/3,429 tests, and the Next.js 16.2.10 build; repository secret scan and live dependency audit also have zero findings. This does not authorize production-sensitive features or public claims.
