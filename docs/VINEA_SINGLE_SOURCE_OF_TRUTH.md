...
Latest verified membership-primary request access hardening: cookie-free staff request detail and document access now resolve the authenticated staff membership context first and use that staff member's authorized primary parish. Membership-resolution errors fail closed; the explicit global oldest-parish compatibility path remains only when no staff client is available. Request-to-parish ownership checks remain unchanged. Focused coverage passed 5 files / 25 tests; the complete 15-check local release contract passed with zero secret findings across 2,842 text files, zero vulnerabilities, every evidence gate, both TypeScript scopes, lint, 836 test files / 3,566 tests, and the credential-free Next.js 16 56-page build. No migration, RLS, production, provider, storage, signed URL, or record action occurred. See `docs/REQUEST_ACCESS_MEMBERSHIP_PRIMARY_FALLBACK_HARDENING_20260720.md`.
