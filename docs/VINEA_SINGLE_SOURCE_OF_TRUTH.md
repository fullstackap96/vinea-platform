...
Current rollout-input boundary (2026-07-14): `docs/CONTROLLED_PRODUCTION_ROLLOUT_HUMAN_INTAKE_20260714.md` is the only current source for the remaining base-rollout owner, channel, production-safe fixture, and exact-window inputs. Its checker currently returns `NO_GO_MISSING_HUMAN_INPUT`; it prints no entered values and can advance only to `READY_FOR_EXPLICIT_APPROVAL`, never production approval. Older role labels or fixture examples are not evidence that a human is available or a production fixture is approved. Promotion, production smoke, migrations, flags, and every separately locked capability remain prohibited until the checkpoint's exact separate approval is supplied.
