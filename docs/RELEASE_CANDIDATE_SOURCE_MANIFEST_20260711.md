...
This gives reviewers a tamper-evident identity for the exact release source surface, but the aggregate alone does not replace an immutable Git commit. Commit `ab8fa9ed101a761fd7ccfb97951edb7670026e53` on `codex/release-candidate-20260711` is the immutable implementation binding for this aggregate. The complete local release gate passed against this source; clean-checkout GitHub Actions and a correctly configured protected-preview smoke remain pending. Before any production deployment, the manifest and release-readiness runner must pass again against the exact approved deployment commit. Excluded non-release artifacts may remain outside the commit and do not authorize production.
