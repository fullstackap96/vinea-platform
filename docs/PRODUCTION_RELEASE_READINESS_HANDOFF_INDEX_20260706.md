...
| Current completed local evidence | `docs/PRODUCTION_RELEASE_READINESS_LOCAL_EVIDENCE_20260707_COMPLETED.md` | Review the latest label-only local verification evidence, including the 2026-07-11 process-clean 12-command refresh with 806 test files and 3,429 tests; this is not production approval. |
...
| Release candidate source manifest | `docs/RELEASE_CANDIDATE_SOURCE_MANIFEST_20260711.md`, `scripts/build-release-candidate-source-manifest.mjs`, and `lib/server/releaseCandidateSourceManifest.test.ts` | Confirm the tested dirty-worktree release surface has a deterministic, secret-safe aggregate identity, then replace it with a clean immutable Git commit and rerun before production. |
| Release candidate commit scope | `docs/RELEASE_CANDIDATE_COMMIT_SCOPE_REVIEW_20260711.md`, `scripts/prepare-release-candidate-commit-scope.mjs`, and `lib/server/releaseCandidateCommitScope.test.ts` | Confirm the proposed branch commit includes source/operational evidence while excluding sales, CSV, generated, temporary, binary, and secret-bearing surfaces; owner approval is required before staging. |
