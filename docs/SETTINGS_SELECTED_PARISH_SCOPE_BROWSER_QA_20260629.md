# Settings Selected-Parish Scope Browser QA
...
This verification checked that the Settings page selected-parish scope label follows the active parish selector after switching between authorized shared-QA parish fixtures.
...
| Check | Result |
|---|---|
| Safe QA staff session could access Settings | Passed |
| Settings page loaded at `/dashboard/settings` | Passed |
| Initial Settings scope label displayed the current active parish | Passed: `Settings are scoped to Vinea QA Google Calendar Parish A.` |
...
| Selecting Parish B updated the Settings scope label after the server refresh settled | Passed: `Settings are scoped to Vinea QA Google Calendar Parish B.` |
| Selecting Parish A updated the selected option | Passed: `Vinea QA Google Calendar Parish A` |
| Selecting Parish A updated the Settings scope label after the server refresh settled | Passed: `Settings are scoped to Vinea QA Google Calendar Parish A.` |
| Page stayed on `/dashboard/settings` | Passed |
...
```json
{
  ...
  "settingsScopeLabels": {
    "initial": "Settings are scoped to Vinea QA Google Calendar Parish A.",
    "afterSelectingParishB": "Settings are scoped to Vinea QA Google Calendar Parish B.",
    "afterSelectingParishA": "Settings are scoped to Vinea QA Google Calendar Parish A."
  },
  "selectedOptions": {
...
- This QA confirmed selected-parish context display only. It did not promote production RLS or change Settings authorization behavior.

## Outcome

Current status: `SETTINGS_SELECTED_PARISH_SCOPE_BROWSER_QA_COMPLETE_SHARED_QA_ONLY`

The Settings selected-parish scope label follows authorized parish switching in the shared-QA browser path.
